...
public class Model {
	...
	public List<Arco> getArchi(){
		//Calcolo peso medio degli archi nel grafo
		...
		//filtro archi e tengo solo quelli con peso>a quello medio
		List<Arco> result=new LinkedList<>();
		for(DefaultWeightedEdge e:this.grafo.edgeSet()) {
			if(this.grafo.getEdgeWeight(e)>pesoMedio)
				result.add(new Arco(this.grafo.getEdgeSource(e), 
	...
	public List<String> trovaPercorso(String sorgente, String destinazione){
		percorsoMigliore=new LinkedList<>();
	...
	private void cerca(String destinazione, List<String> parziale) {
		...
		//proseguo ricorsione
		//scorro vicini dell'ultimo inserito e provo ad aggiungerli uno ad uno
		for(String vicino:Graphs.neighborListOf(grafo, parziale.get(parziale.size()-1))){
			if(!parziale.contains(vicino)) {
				parziale.add(vicino);
				cerca(destinazione, parziale);
				parziale.remove(parziale.size()-1); //è meno dispendioso togliere per indice
			}
		}
	}
}
